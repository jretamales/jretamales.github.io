...
entregan un gran abanico de opciones. Si esto no es suficiente para
convencerte,lo intentaré de hacer reutilizando nuestro anterior ejemplo:
Imaginando que además de tomar `x` en la misma forma como venía originalmente,
nos gustaría probar con diferentes transformaciones: Específicamente, con $x^2$,
$log(x)$ , $e^x$ y $\sqrt(x)$, esto para la etapa de procesamiento de variables.
A su vez, para el Ajuste del modelo, creemos que tanto [Regresión
lineal](http://scikit-
...
(5 transformaciones x 4 modelos) veces el procesamiento en `pandas` y luego
estimación en `sklearn`. Como correctamente supondrás, si utilizas
independientemente ambas librerías como la gran mayoría de los usuarios haría,
deberás entonces construir un laaaargo script, donde tendrás que dedicar
...
las funciones `fit`  y `predict` (los motores verdaderos analíticos de
...
de estrategia de procesamiento + modelo, sera mucho más fácil. 
 
# Custom Transformers 
 
Así, luego de toda la anterior larga introducción hemos llegado al tema central
de este post: Transformaciones de Dataset Customizadas, elemento clave para
poder traducir las transformaciones realizadas en `pandas` a una transformación
que, `sklearn` la pueda entender. Sin este proceso de traducción, `sklearn` no
podrá integrar el procesamiento customizado para nuestro problema, al Pipeline y
obtener los beneficios anteriormente mencionados. (*Nota: Cabe mencionar, que no
necesariamente se debe definir  un nuevo objeto de procesamiento. De hecho,
`sklearn` dispone de múltiples funciones de procesamiento, que pueden resolver
una gran variedad de problemas. En efecto este post busca ilustrar como podemos
aún más esas capacidades, generando una propia que se ajuste a nuestras
...
`sklearn`: `TransformerMixin`. Heredando desde este objeto, nos permitirá luego
...
en el caso de $x^2$, $log(x)$ , $e^x$ y $\sqrt(x)$), esta función debiese
devolver los mismos datos de entrada.
* `transform`: Es la función que transforma la data a partir de los parámetros
estimados en fit (o ninguno si es así el caso) y de la transformación específica
que nosotros deseemos definir. 
...
determinaremos por la  moda de cada columna. La metodología es más bien simple,
importamos las librerías, leemos la data, luego definimos `ModeImputer` para
finalmente ejemplificar su uso. 
...
transformación de dataset compatible con sklearn. Como punto importante a
recalcar es el lugar en la clase en donde obtenemos los parámetros de la data --
el cual en este caso es el diccionario de remplazo `self.replace_with`--, esto
es como parte de la función `fit` y no de `transform`.

Así, la transformación de los datos de tanto entrenamiento como testing estos
parámetros, solo se realizará con los parámetros calculados a partir de los
datos de entrenamiento y no los de testing. La razón de esto surge de la
...
En este sentido, un error común, es por ejemplo utilizar buscar dentro de los
mismos datos de testing los valores para transformar o imputar el dataset.
Cuando, si solo podemos utilizar el testing para evaluar esto no sería posible. 
 
Finalmente ilustramos un caso de uso de `ModeImputer`. Dónde primero separamos
nuestro dataset `df`, en entrenamiento y testing. Luego, ajustamos y imputamos
(transformamos) nuestra data de entrenamiento en una misma línea. Para
finalmente, imputar nuestra dataset de testing con los parámetros estimados en
la linea anterior. 
...
Primero ajustamos a la data para obtener los parametros de reemplazo para luego
imputar el dataset de entrenamiento. Notar qómo ModeImputer hereda de
...
Vieron lo elegante de los codigos `mr.fit()` y `mr.transform()`? Mirándolos por
un par de segundos, no es difícil imaginarse como podríamos customizar y
reutilizarlos para nuestro u otros problemas. Todo en línea, con los métodos
comúnmente utilizados por `sklearn`.
...
destacábamos. Aún así, tengo la esperanza, que en el siguiente post esto hará
más sentido. Allí, construiremos a partir de la bases que aquí hemos descrito,
